...
<body>
			...
			<div class="modal-content">
				<form action="#" id="form">
					...
					<div class="modal-body">
						...
						<div class="form-group">
							<label for="diskripsi_satuan">Deskripsi Satuan <sup>Opsional</sup></label>
							<textarea name="diskripsi_satuan" id="diskripsi_satuan" rows="4" class="form-control"></textarea>
						</div>
					</div>
					<div class="modal-footer">
						<button href="<?= base_url("unit") ?>" class="btn btn-info">Kembali</button>
	...
	<script>
		...
		function editUnit(id_satuan) {
			...
			// load data from ajax
			$.ajax({
				url: "<?= base_url('unit/ajaxedit') ?>/" + id_satuan,
				type: "GET",
				dataType: "JSON",
				success: function(data) {
					$('[name="id_satuan"]').val(data.id_satuan);
					$('[name="nama_satuan"]').val(data.nama_satuan);
					$('[name="diskripsi_satuan"]').val(data.diskripsi_satuan);
					$('#modal_form').modal('show'); // show bootstrap modal
		...
		function deleteUnit(id_satuan) {
			swal({
				...
				if (result) {
					$.ajax({
						url: "<?= base_url('unit/ajaxdelete') ?>/" + id_satuan,
						type: "POST",
						dataType: "JSON",
						success: function(data) {
