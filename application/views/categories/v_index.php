...
<body>
				...
				<form action="#" id="form">
					...
					<div class="modal-body">
						<!-- <div class="form-group">
							...
							<input type="text" class="form-control" id="category_code" name="category_code" value="<?= $kode_kategori ?>" readonly>
						</div> -->
						<div class="form-group">
						...
						<div class="form-group">
							<label for="diskripsi_kategori">Deskripsi Kategori <sup>Opsional</sup></label>
							<textarea name="diskripsi_kategori" id="diskripsi_kategori" rows="4" class="form-control"></textarea>
						</div>
					</div>
					<div class="modal-footer">
						<button href="<?= base_url("category") ?>" class="btn btn-info">Kembali</button>
	...
	<script>
		...
		function editCategory(id_kategori) {
			saveMethod = 'update';
			$('#form')[0].reset(); // reset form on modals
			$('.form-control').removeClass('is-invalid'); // clear error class
			$('.invalid-feedback').empty(); // clear error string
			...
			// load data from ajax
			$.ajax({
				url: "<?= base_url('category/ajaxedit') ?>/" + id_kategori,
				type: "GET",
				dataType: "JSON",
				success: function(data) {
					...
					$('[name="diskripsi_kategori"]').val(data.diskripsi_kategori);
					$('#modal_form').modal('show'); // show bootstrap modal
		...
		function deleteCategory(id_kategori) {
			swal({
				...
				if (result) {
					$.ajax({
						url: "<?= base_url('category/ajaxdelete') ?>/" + id_kategori,
						type: "POST",
						dataType: "JSON",
						success: function(data) {
