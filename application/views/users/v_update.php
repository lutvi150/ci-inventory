										...
										<div class="col-8 mx-auto">
											<form action="<?=base_url("user/update/" . $user["id_user"])?>" method="post" enctype="multipart/form-data">
												<div class="form-group">
													...
													<input type="text" class="form-control <?=form_error('user_name') ? 'is-invalid' : '';?>" name="user_name" id="user_name" placeholder="Nama User" value="<?=$user["nama_user"]?>">
													<?=form_error('user_name', '<div class="invalid-feedback font-weight-bold pl-1">', '</div>')?>
												</div>
												<div class="form-group">
													<label for="user_email">Email User</label>
													<input type="text" class="form-control <?=form_error('user_email') ? 'is-invalid' : '';?>" name="user_email" id="user_email" placeholder="Email User" value="<?=$user["email_user"]?>">
