...
class Category extends CI_Controller
{
	...
	public function ajaxList()
	{
		...
		foreach ($list as $category) {
			...
			$row[] = $category->diskripsi_kategori;
			$row[] = date('d F Y', strtotime($category->created_at));

			$row[] = '<a class="btn btn-icon btn-warning" href="javascript:void(0)" title="Edit" onclick="editCategory(' . "'" . $category->id_kategori . "'" . ')"><i class="fas fa-pencil-alt"></i></a>
			<a class="btn btn-icon btn-danger" href="javascript:void(0)" title="Delete" onclick="deleteCategory(' . "'" . $category->id_kategori . "'" . ')"><i class="fas fa-trash"></i></a>';
	...
	public function ajaxAdd()
	{
		...
		$data = array(
			...
			"diskripsi_kategori" => $this->input->post("diskripsi_kategori")
		);
	...
	public function ajaxUpdate()
	{
		...
		$this->Category_model->update(array('id_kategori' => $this->input->post('id_kategori')), $data);
		echo json_encode(array('status' => TRUE));
	}
	...
	private function _validateForm()
	{
		...
		if ($this->input->post("nama_kategori") == '') {
			$data['input_error'][] = 'nama_kategori';
